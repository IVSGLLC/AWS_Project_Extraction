...
class PartsParser():
    ...
    def parse(self,fileDataAsStr):
       ...
       # REFER# SALE-TYPE CP-TAX-SALE$ TOTAL$ CP-PART-SALE$ CP-SUBL-SALE$ HPHONE ID-SUPP           
        ...
        SKIP_LINE_CONTAINS_TEXT="LIST WIP REFER# SHIPPED OPEN-DATE CUST^SORT WIP REFER# SHIPPED OPEN-DATE CUST^ ITEMS SELECTED^GET-LIST^REFER#.. SHIPPED OPEN-DATE^ITEM SELECTED^ITEMS SELECTED^ITEM LISTED^ITEMS LISTED^REFER# SALE-TYPE^SALE $             SALE $   SALE $^CATALOGED;^PARTS REF# Q.O. Q.S. Q.B. ^Q.O. Q.S. Q.B. PART NO......... DESC...^COUNT PRIVLIB^' NOT ON FILE"
        SKIP_LINE_STARTWITH_TEXT='>LIST^PAGE^:SORT^:^login^password^>SAVE-LIST^>^>SORT^:SELECT^:SSELECT^REFER#.. SHIPPED OPEN-DATE^REFER#.. SALES TYPE.... CP TAX^REFER#.... Q.O. Q.S. Q.B. PART NO'
        ...
        while row:
            ...
            if row.strip() != "" and wip_2==True and not row=="\n" and not row.startswith(tuple(skip_line_startwith_list)) and not (list(filter(row.__contains__, skip_line_contains_list)) != []):
                if len(row[0:8].strip())==0 :
                    if len(row[73:87].strip())>0:
                        dict_WIP_2["PO#"][(len(dict_WIP_2["PO#"])-1)] = (dict_WIP_2["PO#"][(len(dict_WIP_2["PO#"])-1)] + str(row[73:87].strip())) 
                    if len(row[104:].strip())>0:
                        dict_WIP_2["EMAIL"][(len(dict_WIP_2["EMAIL"])-1)] = (dict_WIP_2["EMAIL"][(len(dict_WIP_2["EMAIL"])-1)] + str(row[104:].strip()))                         
                else:
                    ...
                    dict_WIP_2["TOTAL$"].append(str(row[33:45].strip())) 
                    dict_WIP_2["CP-PART-SALE$"].append(str(row[46:54].strip()))
                    dict_WIP_2["CP-SUBL-SALE$"].append(str(row[55:63].strip()))
                    po=str(row[64:87].strip())
                    po=po.replace(row[9:23].strip(), "",1)
                    dict_WIP_2["PO#"].append(po)
                    dict_WIP_2["HPHONE"].append(str(row[88:100].strip()))
                    dict_WIP_2["PPFLAG"].append(str(row[101:103].strip()))
                    dict_WIP_2["EMAIL"].append(str(row[104:].strip()))
            ...
            if row.strip() != "" and parts==True and not row=="\n" and not row.startswith(tuple(skip_line_startwith_list)) and not (list(filter(row.__contains__, skip_line_contains_list)) != []):
                if len(row[0:10].strip())==0 :
                    ...
                    dict_parts["SVIA"].append(str(row[105:113].strip()))
                    dict_parts["EMP#"].append(str(row[114:121].strip()))
                    dict_parts["SALE"].append(str(row[122:130].strip()))
                    dict_parts["LINE"].append(str(row[131:].strip()))
