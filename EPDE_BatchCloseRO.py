...
class BatchCloseRO(object):
    logger=LogManger()
    err_handler=ErrorHandler()
    region='us-east-1'
    table_Per_Store_BatchCloseRO=1
    def __init__(self):        
        BatchCloseRO.region="us-east-1"
        BatchCloseRO.table_Per_Store_BatchCloseRO=1
        
    def __init__(self,region,table_Per_Store_BatchCloseRO=1):  
        BatchCloseRO.region=region
        BatchCloseRO.table_Per_Store_BatchCloseRO=table_Per_Store_BatchCloseRO
    ...
    def GetTableName(self,store_code):         
        ...
        return TableName
    @classmethod
    def GetBatchCloseROEntry(self,store_code,fileId):
        try:
            _moduleNM="BatchCloseRO"
            _functionNM="GetBatchCloseROEntry"
            self.logger.debug("GetBatchCloseROEntry>> store_code="+str(store_code)+",fileId="+str(fileId))            
    ...
    @classmethod
    def GetBatchCloseROProcessLogs(self,store_code,fileId):
        process_logs=[]
        try:
            _moduleNM="BatchCloseRO"
            _functionNM="GetBatchCloseROProcessLogs"
            self.logger.debug("GetBatchCloseROProcessLogs>> store_code="+str(store_code)+",fileId="+str(fileId))            
            dynamodb = boto3.resource('dynamodb', region_name=BatchCloseRO.region)
            TableName=self.GetTableName(store_code)
            table = dynamodb.Table(TableName)
            response = table.get_item(Key={'file_id': fileId},ConsistentRead=True)
            try:
               ...
               return process_logs  
    @classmethod
    def SaveBatchCloseRO(self,post_dict):  
        _moduleNM="BatchCloseRO"
        _functionNM="SaveBatchCloseRO"
        try:  
            isUpdate=False         
            self.logger.debug("SaveBatchCloseRO >> post_dict="+str(post_dict))
            if post_dict['file_id'] is None:
                fileId=self.GetFileId()            
                res=self.GetBatchCloseRORequest(fileId,post_dict['request_json'],post_dict['store_code'],post_dict['is_spc_ins'])
                if res['status']==False:
                   return res
            ...
            try:
                ...
                is_timeout=False
            if not 'root_file_id' in post_dict:
            ...
            if isUpdate==False:
                self.logger.debug("BatchCloseRO Request Created file_id:"+fileId+",request_status:"+post_dict['request_status'])
            else:
                self.logger.debug("BatchCloseRO Request Updated file_id:"+fileId+",request_status:"+post_dict['request_status'])
           
            #json.dumps({'b': 1, 'a': 2})
            return { "status":True,"file_id": fileId,"batchclosero_request":requestJSON }          
        except Exception as e:
            return self.err_handler.HandleGeneralError(moduleNM=_moduleNM,functionNM=_functionNM)
    @classmethod
    def GetFileId(self):
    ...
    @classmethod
    def GetBatchCloseRORequest(self,fileId,Payments,store_code,is_spc_ins):
        _moduleNM="BatchCloseRO"
        _functionNM="GetBatchCloseRORequest"
        try:
            if not type(Payments) is list:
                ...
                if "api" in Payments:   
                    Payments["api"]="batchclosero"
                else:
                    att={"api": "batchclosero"}
                    Payments.update(att)  
    ...
    def UpdateBatchCloseRO(self,store_code,fileId,req_status,response_json):
        _moduleNM="BatchCloseRO"
        _functionNM="UpdateBatchCloseRO"
        ct = datetime.datetime.now()
        res_ts = ct.timestamp()
        try:
            self.logger.debug("UpdateBatchCloseRO>> store_code="+str(store_code)+",req_status:"+req_status+",fileId:"+fileId)            
    ...
    @classmethod
    def UpdateBatchCloseROTimeOut(self,store_code,fileId,req_status,response_json):
        _moduleNM="BatchCloseRO"
        _functionNM="UpdateBatchCloseROTimeOut"
        ct = datetime.datetime.now()
        res_ts = ct.timestamp()
        try:
            self.logger.debug("UpdateBatchCloseROTimeOut>> store_code="+str(store_code)+",req_status:"+req_status+",fileId:"+fileId)            
            #Added 11/30/2022 for keep Processing History for each BatchClose RO Request    
            process_logs=self.GetBatchCloseROProcessLogs(store_code=store_code,fileId=fileId)
            ...
            return self.err_handler.HandleGeneralError(moduleNM=_moduleNM,functionNM=_functionNM)
    @classmethod
    def UpdateBatchCloseRORequestStatus(self,store_code,fileId,req_status):
        _moduleNM="BatchCloseRO"
        _functionNM="UpdateBatchCloseRORequestStatus"
        ct = datetime.datetime.now()
        res_ts = ct.timestamp()
        try:
            self.logger.debug("UpdateBatchCloseRORequestStatus>> store_code="+store_code+",req_status:"+req_status+",fileId:"+fileId)             
            #Added 11/30/2022 for keep Processing History for each BatchClose RO Request    
            process_logs=self.GetBatchCloseROProcessLogs(store_code=store_code,fileId=fileId)
            comments="update request_status ="+str(req_status)
            process_log={"log_time":str(res_ts), "request_status":str(req_status),"comments":comments}
    ...
    @classmethod
    def UpdateBatchCloseRORetryCount(self,store_code,file_id,retry_count,ro_status):
        _moduleNM="BatchCloseRO"
        _functionNM="UpdateBatchCloseRORetryCount"
        try:
            self.logger.debug("UpdateBatchCloseRORetryCount file_id:"+str(file_id))            
            ct = datetime.datetime.now()
            res_ts = ct.timestamp()
            ...
            return self.err_handler.HandleGeneralError(moduleNM=_moduleNM,functionNM=_functionNM)
    @classmethod
    def UpdateBatchCloseRORetryCountRequestStatusTimeOut(self,store_code,file_id,retry_count,ro_status,req_sts,response_json):
        _moduleNM="BatchCloseRO"
        _functionNM="UpdateBatchCloseRORetryCountRequestStatusTimeOut"
        try:
            self.logger.debug("UpdateBatchCloseRORetryCountRequestStatusTimeOut file_id:"+str(file_id))            
            ct = datetime.datetime.now()
            res_ts = ct.timestamp()
            #Added 11/30/2022 for keep Processing History for each BatchClose RO Request    
            process_logs=self.GetBatchCloseROProcessLogs(store_code=store_code,fileId=file_id)
            ...
            response = table.update_item(
                            ...
                            ExpressionAttributeValues={                                
                                ...
                                ':tt':True,
                                ':rspj':self.writeLargeData(data=response_json),
                                ':plogs':self.writeLargeData(process_logs),
                            },
                            ReturnValues="UPDATED_NEW"         
                    )
            return { "status":True,"response": response } 
    ...
    @classmethod
    def Validate_BatchCloseRO_inputs(self,_payments,auth_json,store_code,tablePerStoreRODetail=1):
        _moduleNM="BatchCloseRO"
        _functionNM="Validate_BatchCloseRO_inputs"
        paymentsJSON=_payments
        paymentsList=[]
        payments=None
        ...
        count=0
        finalPaymentList=[]
        batchCloseROResponseList=[]
        ro_detail_list=[]
        autoCloseRO=False
        try:
            self.logger.debug("Validate_BatchCloseRO_inputs >>store_code:"+str(store_code)+",_payments="+str(_payments))
            
            if (paymentsJSON is None ): 
                res=res_handler.GetErrorResponseJSON(code=308,auth_json=auth_json)
                res["batchCloseROResponses"]: None  
                res["autoCloseRO"]: False       
                return res
            else:
                paymentsList= paymentsJSON['batchCloseRORequests']
                autoCloseRO=False
                if 'autoCloseRO' in paymentsJSON:
                    autoCloseRO= paymentsJSON['autoCloseRO']
                
                rOrder=RepairOrder(BatchCloseRO.region,tablePerStoreRODetail)
                ...
                if len(paymentsList)==0 and autoCloseRO==True:
                    ...
                    if ro_resp['status']==True:
                        ...
                        batchclosero_list = [existing_item for existing_item in roList if ( str(existing_item['status']).upper() == 'C97' and (str(existing_item['amount_due']).strip()  == "" or str(existing_item['amount_due']).strip()  == "0" or str(existing_item['amount_due']).strip()  == "0.0"))]
                    issued = datetime.datetime.now()
                    create_date = issued.strftime("%m-%d-%Y")                      
                    for batchRO in batchclosero_list:
                        self.logger.debug("batchRO="+str(batchRO))
                        b_json={
                                    "documentId": batchRO['document_id'],
                                    "customerId": batchRO['customer_id'],
                                    "employeeId":batchRO['employee_id'] ,
                                    "status": batchRO['status'],
                                    "transactionDate": create_date,
                                    "paymentDetails": [
                                        {
                                        "amount": batchRO['amount_due'],
                                        "warrantyDue": batchRO['warranty_due'],
                    ...
                    if ro_resp['status']==True:
                        roList=ro_resp['items']
                        batchclosero_list=roList 

                if ( len(paymentsList)==0 ): 
                    self.logger.debug("Error: no paymentsList =")
                    res=res_handler.GetErrorResponseJSON(code=382,auth_json=auth_json)
                    res["batchCloseROResponses"]: None    
                    res["autoCloseRO"]: autoCloseRO      
                    return res
                store_detail=None
                app_client=AppClient()
                store_detail_resp=app_client.GetStoreDetail(store_code,BatchCloseRO.region)  
                if store_detail_resp['status']:  
                    store_detail=store_detail_resp['item']
                    if not store_detail is None:                                    
                        if 'batchCloseRO' in store_detail and store_detail['batchCloseRO']==False:
                            self.logger.debug("Error:batchCloseRO= False")
                            res=res_handler.GetErrorResponseJSON(code=383,auth_json=auth_json)
                            res["batchCloseROResponses"]: None    
                            res["autoCloseRO"]: autoCloseRO      
                            return res
                for payments in paymentsList:   
                    ...
                    if (document_id is None ) or document_id == "" or len(document_id.strip())==0:
                        self.logger.debug("Error: no document_id =")
                        errorList.append({'index':count,'code':310})
                        count=count+1 
                        continue
                    ...
                    if (employee_id is None ) or employee_id == "" or len(employee_id.strip())==0:
                        self.logger.debug("Error: no employee_id =")
                        errorList.append({'index':count,'code':320})
                        count=count+1 
                        continue
                    ...
                    if (status is None ) or status == "" or len(status.strip())==0:
                        self.logger.debug("Error: no status =")
                        errorList.append({'index':count,'code':315})
                        count=count+1 
                        continue
                        #return res_handler.GetErrorResponseJSON(code=315,auth_json=auth_json)    
                    if  status.lower() != "c97" :
                        self.logger.debug("Error: no status c97 =")
                        list=[ str(status)]
                        errorList.append({'index':count,'code':316,'args':list})
                        count=count+1 
                        continue
                    ...
                    if paymentDetails is None or len(paymentDetails)==0:
                        self.logger.debug("Error: no paymentDetails =")
                        errorList.append({'index':count,'code':327})
                        count=count+1 
                        continue
                        #return res_handler.GetErrorResponseJSON(code=327,auth_json=auth_json) 
                    
                    ro_list = [existing_item for existing_item in batchclosero_list if ( str(existing_item['document_id'])  == document_id )]
                    if len(ro_list)>0:
                        ro_detail= ro_list[0]   #rOrder.GetRODetail(store_code,document_id,'RO') 
                        
                    else:
                        self.logger.debug("Error: no roDetail =")
                        #save not posted request                
                        error_code=341
                        errorList.append({'index':count,'code':error_code})
                        count=count+1 
                        continue
                    ...
                    if  dms_status.lower() != "c97" :
                        self.logger.debug("Error: no dms_status c97 =")
                        list=[ str(status),str(dms_status)]
                        errorList.append({'index':count,'code':362,'args':list})
                        count=count+1 
                        continue
                    ...
                    if (employee_no is not None ) and employee_id != employee_no :
                        self.logger.debug("Error: no employee_no =")
                        list=[employee_id]
                        errorList.append({'index':count,'code':314,'args':list})
                        count=count+1 
                        continue
                    ...
                    for detail in paymentDetails:
                        ...
                        if (amountStr is None ) or amountStr == "" or len(amountStr.strip())==0:
                            amountStr="0.0"
                            #errorList.append({'index':count,'code':319})
                            #isError=True
                            #self.logger.debug("Error: no amountStr 0 =")
                            #break
                        ...
                        try : 
                            ...
                            res = False
                        if not res:
                                self.logger.debug("Error: no amountStr 1 =")
                                list=[amountStr]
                                errorList.append({'index':count,'code':309,'args':list})
                                isError=True
                                break
                        ...
                        if (warrantyDueStr is not None ) and  warrantyDueStr != "" and  len(warrantyDueStr.strip())>0:
                           ...
                           # store_detail=store_detail_resp['item']
                            ...
                            """ posPayType=None
                            if 'is_paytype_mapping_activated_batchclosero' in store_detail and store_detail['is_paytype_mapping_activated_batchclose'] == True:
                                    posPayType=None
                                    try:
                            ...
                            if 'is_transactionsource_mapping_activated_batchclosero' in store_detail  and store_detail['is_transactionsource_mapping_activated_batchclosero'] == True:
                    ...
                    #loop end
                    if isError == True: 
                        self.logger.debug("Error: isError =True=")
                        count=count+1 
                        continue
                    ...
                    try : 
                        ...
                        if amountdue != totalAmount:
                            self.logger.debug("Error: amountdue not total amount =")
                            list=[str(totalAmount),str(amountdue)]
                            errorList.append({'index':count,'code':334,'args':list})
                            count=count+1
                            continue
                            ...
                            amountdue=0
                    if amountdue>0:
                        self.logger.debug("Error: amountdue not 0 =")
                        errorList.append({'index':count,'code':381})
                        count=count+1
                        continue
                        #return res_handler.GetFormattedErrorResponseJSON(code=381,auth_json=auth_json,args=list)
                    
                    count=count+1
                    finalPaymentList.append(payments)
                    ro_detail_list.append(ro_detail)
                    batchCloseROResponseList.append({  "documentId": payments['documentId'],"paymentResponse": paymentresponse  })
            if len(errorList)>0:
                batchCloseROErorResponseList=[]
                errorList_res=[]
                for err in errorList:
                    payments=paymentsList[err['index']]
                    if 'args' in err:
                        msg=self.err_handler.GetErrorFormattedMessage(err['code'],args=err['args'])
                    else:
                        msg=self.err_handler.GetErrorMessage(err['code'])
                    errorList_res.append({"code":err['code'],"message":msg})
                    try:                     
                        paymentDetails=payments['paymentDetails']
                    except KeyError:
                         paymentDetails=None
                    paymentDetailsList=None
                    if paymentDetails is not None and  len(paymentDetails)>0:
                        paymentDetailsList=[]
                        for detail in paymentDetails:
                            pRes={
                                        "amount": detail['amount'],
                            ...
                            paymentDetailsList.append(pRes)  
                     
                    paymentresponse={  "documentId": payments['documentId'],"paymentResponse": paymentDetailsList, "errorCode":err['code'] }
                    batchCloseROErorResponseList.append(paymentresponse)
                 
                return {
                    "responseCode": -1,                     
                    "batchCloseROResponses": batchCloseROErorResponseList,
                    "autoCloseRO":autoCloseRO,
                    "errorList":errorList_res,
                    "auth_token": auth_json 
                    }
            else:
                resjson={
                    "responseCode": 0,                     
                    "batchCloseROResponses": batchCloseROResponseList,
                    "autoCloseRO":autoCloseRO,
                    "auth_token": auth_json 
                    }
            return { "responseCode":0,"batchclosero_request": finalPaymentList,"ro_detail_list":ro_detail_list,"batchclosero_response":resjson,"store_detail":store_detail } 
        except Exception as e:            
    ...
    @classmethod
    def PollBatchCloseROResponse(self,store_code,file_id,auth_json,RequestTimeOutInSecond=20,ResponseQueueName="EPDE_PostPayment_Response.fifo",region="us-east-1",MaxMsgCount=1,waitTimeInSecond=20):
       # _moduleNM="BatchCloseRO"
       # _functionNM="PollBatchCloseROResponse"
       # self.err_handler.appInfo(moduleNM=_moduleNM,functionNM=_functionNM) 
        ...
        self.logger.debug("PollBatchCloseROResponse time out:"+str(timeOut))
        while time_c<=timeOut:
            if False:    
                entry=self.GetBatchCloseROEntry(store_code,file_id)
                if entry['status'] :
                    request= entry['item']
                ...
                response_json=sqs.fetch_response("batchclosero",file_id)
    ...
    @classmethod
    def buildBatchCloseROObject(self,validate_resp,store_code,request_status,req_time,res_time,Is_WaitForResponse):
        post_dict={}    
        ...
        try:
            response_json =  validate_resp['batchclosero_response']  
            post_dict['response_json']=response_json
        except:  
            post_dict['response_json']=None
       
        try:
            request_json=validate_resp['batchclosero_request']
        ...
        try:
            request_json=post_dict['request_json']
    ...
    def isRetryAllow(self,store_detail,ro_detail):
        ...
        #self.err_handler.appInfo(moduleNM=_moduleNM,functionNM=_functionNM) 
        initiateRetry=False
        if 'is_retry_active_batchclosero' in store_detail :
            isRetryActive= store_detail['is_retry_active_batchclosero']
            MaxRetryCount=0
            if 'retry_max_count_batchclosero' in store_detail:
                MaxRetryCount=int(store_detail['retry_max_count_batchclosero'])

            if isRetryActive and MaxRetryCount>0:
                initiateRetry=True
                """ isAllowCloseAllRO=store_detail['close_all_RO_flag_batchclosero']
    ...
    @classmethod
    def GetBatchCloseRORequestList(self,store_code,create_date):
        _moduleNM="BatchCloseRO"
        _functionNM="GetBatchCloseRORequestList"
        try:
            self.logger.debug("GetBatchCloseRORequestList>> store_code="+str(store_code)+",create_date="+str(create_date))
    ...
    @classmethod
    def GetBatchCloseRORereconciliationReport(self,store_code,create_date,tablePerStoreRoDetail=1):
        _moduleNM="BatchManager"
        _functionNM="GetBatchCloseRORereconciliationReport"
        try:           
            self.logger.debug("GetBatchCloseRORereconciliationReport>> store_code:"+str(store_code)+",create_date="+create_date)
            app_client=AppClient()
            store_resp=app_client.GetStoreDetail(store_code,BatchCloseRO.region)
            if store_resp['status']==False:
               self.logger.debug("GetStoreDetail status false:") 
               return store_resp
            rOrder=RepairOrder(BatchCloseRO.region,tablePerStoreRoDetail)
            #create_date=''
            post_resp=self.GetBatchCloseRORequestList(store_code,create_date)
            ...
            if post_resp['status']==False:
                self.logger.debug("GetBatchCloseRORequestList status false:") 
                return post_resp
            else:
    ...
    @classmethod
    def GetBatchCloseRORereconciliationReportPendingOnly(self,store_code,create_date,C97Pending_Since_inSecond=15,tablePerStoreRoDetail=1):
        _moduleNM="BatchManager"
        _functionNM="GetBatchCloseRORereconciliationReportPendingOnly"
        try:           
            self.logger.debug("GetBatchCloseRORereconciliationReportPendingOnly>> store_code:"+str(store_code)+",create_date="+create_date+",C97Pending_Since_inSecond="+str(C97Pending_Since_inSecond))
            app_client=AppClient()
            store_resp=app_client.GetStoreDetail(store_code,BatchCloseRO.region)            
            if store_resp['status']==False:
            ...
            if post_resp['status']==False:
                ...
                excelName=store_code+"_BatchCloseROPendingC97CallLog_"+".xlsx"
