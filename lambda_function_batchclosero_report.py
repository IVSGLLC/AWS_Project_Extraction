                    ...
                    if stores_resp!=None and  stores_resp['status']== True:
                        ...
                        for store in stores:
                            ...
                            if 'batchCloseRO' in store and store['batchCloseRO']==True:  
                                ...
                                date=datetime.datetime.combine(issued, datetime.datetime.min.time())
                                logger.info("date:"+str(date))
                                create_date=str(date.timestamp())  
                                logger.info("create_date:"+str(create_date))                  
                                ro_state_resp= post_mgr.GetBatchCloseROReport(store_code,create_date,tableperstore) 
...
def getTemaplate(table,create_dt,head_bg_color,body_bg_color,group_name):    
    ...
    x=f"""<!DOCTYPE html>
        <html>
        <head>
        <title>BatchCloseRO Report</title>
        ...
        <p>Hi All,</p>
        <p>Summary of EPDE BatchCloseRO for {group_name} Stores as of {create_dt}.</p>
        <p>{table}</p>
        </body>
        </html>""" 
    return x
...
def send_email(group,ro_state_list,attachments,region,add_from="sanjay.sahu@ivsgllc.com",istest=False):
    ...
    try:
        ...
        #to_list=['sanjay.sahu@ivsgllc.com'] 
        html_table_header= f"""
                ...
                <thead>
                    <tr>
                        <th>Store</th>
                        <th>logon</th>
                        <th>RO Submitted for Close</th>
                        <th>RO Closed C98</th>
                        <th>RO Not Closed C97</th>"
                        <th>RO In Queue</th>
                        <th>RO Timeout</th>
                    </tr>
                </thead>"""
        ...
        subject='EPDE BatchCloseRO Status Report -'+create_date
