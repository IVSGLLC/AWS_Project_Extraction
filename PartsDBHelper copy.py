...
class PartsDBHelper():
        ...
        try:
            ...
            for  row in df_ro:
                ...
                if len(dict_parts)>0:
                    ...
                    #END FOR PARTS
                    try:  
                        parts = sorted(parts, key=lambda d: int(d['line']))   
                    except:
                        ""   
                #END IF PARTS
                totalSaleTax=0
            ...
            logger.info("Parts Data has been inserted successfuly in table ["+TableName+"] Record Count=."+str(recount))
    ...
    def Save_TKParts(self,store_code,json_parts,client_id):
        ...
        try:
            ...
            delta=et-ct
            logger.debug("after calling lambda total_seconds="+str(delta.total_seconds())) 
            logger.info("TEKION Parts Data has been inserted successfuly in table ["+TableName+"] Record Count=."+str(recount))
            return { "operation_status":"SUCCESS","total_item_count":str(recount),"total_item_parsed":str(len(items)),"items":items} 
        except Exception as e:
            # Return failed record's sequence number
