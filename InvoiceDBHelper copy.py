...
class InvoiceDBHelper():
    ...
    def SaveInvoice(self,store_code,df,client_id):
        ...
        try:
            ...
            for  row in df_ro:
                ...
                if len(dict_labor)>0:                    
                    ...
                    for  rowLabor in dict_labor:
                        ...
                        try:  
                            opCodes = sorted(opCodes, key=lambda d: int(d['seqNo']))   
                        except:
                            ""      
                        serviceDetail={
                                    "lineCode":lineCode,
                    ...
                    #END FOR 
                servicedetails=sorted(servicedetails, key=itemgetter('lineCode'))  
                #end labor condition 
                #lineCode=""
                ...
                try:
                    roMls = sorted(roMls, key=lambda d: int(d['seqNo'])) 
                except:
                    ""  
            ...
            logger.info("Invoice Data has been inserted successfuly in table ["+TableName+"] Record Count=."+str(recount))
    ...
    def GetPartsDataList(self,df_parts,referNumber,lineCode, seqList):
        ...
        if len(dict_parts)>0:
            ...
            for rowParts in dict_parts :  
                ...
                parts.append(part)
        try: 
            parts = sorted(parts, key=lambda d: int(d['seqNo']))  
        except:
            ""
        return parts
    @classmethod
