...
def lambda_handler(event, context):
     ...
     try:
          _moduleNM="EPDE_INVENTORY_STATUS_Lambda"
          ...
          if auth_detail_resp['status']== True:
               ...
               """  try:
                    ...
                    appt=None
               try:
                    apptDate= queryStringParameters["apptDate"]
                    noFilters=False        
               except:
                    apptDate=None
               try:
               ...
               activeServicePlans=[]
               logger.debug("store_code="+store_code+",toDate="+str(toDate)+",fromDate="+str(fromDate)+",plan="+str(gpsServicePlan)+",vinList="+str(vinList)+",stockFilter="+str(stockFilter)+",fromEntryDate="+str(fromEntryDate)+",toEntryDate="+str(toEntryDate))                    
               inventory_manager= Inventory(region,tablePerStore)       
               validateResp=inventory_manager.Validate_inputs(store_code=store_code,vinList=vinList,gpsServicePlan=gpsServicePlan,fromDate=fromDate,toDate=toDate,auth_json=auth_json,stockFlag=stockFilter,fromEntryDate=fromEntryDate,toEntryDate= toEntryDate)
               if validateResp['responseCode']== 0:
                    ...
                    if  noFilters :
                        ...
                        ro_resp=inventory_manager.GetFilteredList(store_code=store_code,vinList=vinList,gpsServicePlan=gpsServicePlan,fromDate=fromDate,toDate=toDate,activeServicePlans=activeServicePlans,stockflag=stockFilter,fromEntryDate=fromEntryDate,toEntryDate=toEntryDate,last_key=lastEvaluatedKey,page_size=limit)                     
                    if ro_resp['status']== True: 
                              ...
                              try:
                                   ...
                                   ""
                              pre_resp=inventory_manager.prepareOutputJSONList(items=items,vinsList=vinList,isRepairOrderAPI=False,plan=gpsServicePlan,activePlanList=activeServicePlans,fromDate=fromDate,toDate=toDate,stockflag=stockFilter,fromEntryDate=fromEntryDate,toEntryDate=toEntryDate)
                              if pre_resp['status']== True:
                                   ...
                                   logger.info("store_code="+store_code+"_INVENTORY_STATUS_LIST_JSON :"+str(res_json)) 
                              else:
