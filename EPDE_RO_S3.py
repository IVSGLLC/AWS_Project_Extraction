...
class RepairOrder(object):
    ...
    @classmethod
    def GetROListS3(self,store_code,document_type,open_date_time="",last_key=None,page_size=-1):
        _moduleNM="RepairOrder"
        _functionNM="GetROListS3"
        try:           
            #self.err_handler.appInfo(moduleNM=_moduleNM,functionNM=_functionNM)       
            self.logger.debug("GetROLists3>> store_code:"+str(store_code)+",open_date_time:"+str(open_date_time)+",document_type"+str(document_type))
            bucketName='epde' 
            s3=S3Manager(RepairOrder.region)
            TableName=self.getTableName(store_code)            
            dataS3Location="store_data"+"/"+store_code+"/"+TableName+".json"
            LastEvaluatedKey=None
            try:
                json_str=s3.read_s3_file(bucketName=bucketName,fileName=dataS3Location)
                self.logger.debug("GetROLists3>>read_s3_file="+str(json_str))  
                items=[]
                if json_str is not None and len(json_str)>0:
                    items=json.loads(json_str)
                    self.logger.debug("GetROLists3>>counts="+str(len(items)))           
                return { "status":True,"items": items,"LastEvaluatedKey":LastEvaluatedKey }
            except:
                return { "status":True,"items": [],"LastEvaluatedKey":LastEvaluatedKey }          
        except Exception as e:
