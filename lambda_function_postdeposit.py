...
def lambda_handler(event, context):
    ...
    try:
            ...
            req_ts = ct.timestamp()
            _moduleNM="EPDE_Lambda_PostDeposit"
            ...
            logger.debug("tableperstore_post_deposit..."+str(tableperstore_post_deposit))
            #wForResp=str(config['WAIT_FOR_DEPOSITS_RESPONSE']  )
            Is_WaitForResponse=False
                    ...
                    if save_resp['status']== True: 
                        ...
                        if sqs_resp['status'] == True:
                            """ if Is_WaitForResponse:  
                                pollres_json=post_manager.PollPostDepositResponse(store_code,save_resp['file_id'],auth_json,requestWaitTImeOutInMin,res_queueNM,region,res_msg_count,res_wait_time)  
                                                            
                                #pollres_json=post_manager.PollPostDepositResponse(store_code,save_resp['file_id'],auth_json,requestWaitTImeOutInMin) 
