...
class WIPParser():
    ...
    def parse(self,fileDataAsStr): 
        ...
        wip_status_line=str('REFER#.. STATUS PRINT-TIME............... CLOSED. COMMENT................. HOME PHONE').split("^")
        
        #RO...... OP-CODE... LBR-TYPE
        wip_opcode_line=str('RO...... OP-CODE').split("^")
        ...
        skip_line_contains_list=str("ITEMS SELECTED^REFER# OPEN-DATE EPDE.TOTAL COMMENTS YEAR MAKE MODEL ID-SUPP^REFER# EPDE.CUST CUST-N1 CUST-N3 CITY-STATE-ZIP SA-R5 SA-NAME VIN ID-SUPP^LABOR RONUM EPDE.EMAIL EPDE.PHONE TAG-NO EPDE.WT.TOTAL ID-SUPP^REFER# RO-STATUS PRINT.TIME CLOSED COMMENT HPHONE ID-SUPP^' NOT ON FILE^LABOR-OPS RO OP-CODE").split("^")
        #>SORT^PAGE^:GET-LIST WIP^:SSELECT WIP WITH VIN^>SAVE-LIST WIP^login^password
        skip_line_startwith_list=str('>LIST^>SORT^PAGE^:GET-LIST^:SSELECT^>SAVE-LIST^login^password^REFER#.. OPEN-DATE TOTAL$... COMMENTS.....^REFER#.. EPDE.CUST... CUSTOMER LINE1.......... CUSTOMER LINE3.......... CITY-STATE-ZIP..................... SA-R.. SA-NAME.................. SERIAL NO........^REFER#.. STATUS PRINT-TIME............... CLOSED. COMMENT................. HOME PHONE^RONUM... EMAIL.............................. PHONE....... TAG-NO. WP$.........^RO...... OP-CODE').split("^")
        ...
        while row:
            ...
            if row.startswith(tuple(wip_email_line)):           
                ...
                wip_email=True 
            if row.startswith(tuple(wip_opcode_line)):
                wip_opcode=True              
            if list(filter(row.__contains__, line_end_text)) != []:
                ...
                if(wip_opcode==True): wip_opcode=False 
            ...
            if wip_cust==True and not row=="\n" and not row.startswith(tuple(skip_line_startwith_list)) and not (list(filter(row.__contains__, skip_line_contains_list)) != []):
                ...
                if len(row[0:8].strip())==0 and len(row[9:21].strip())==0:
                    if len(row[22:46].strip())>0:
                        dict_cust["CUSTOMER LINE1"][(len(dict_cust["CUSTOMER LINE1"])-1)] = (dict_cust["CUSTOMER LINE1"][(len(dict_cust["CUSTOMER LINE1"])-1)] + str(row[22:46].rtrip()))
                    if len(row[47:71].strip())>0:
                        dict_cust["CUSTOMER LINE3"][(len(dict_cust["CUSTOMER LINE3"])-1)] = (dict_cust["CUSTOMER LINE3"][(len(dict_cust["CUSTOMER LINE3"])-1)] + str(row[47:71].rtrip())) 
                    ...
                    if len(row[115:140].strip())>0:
                        dict_cust["SR-NAME"][(len(dict_cust["SR-NAME"])-1)] = (dict_cust["SR-NAME"][(len(dict_cust["SR-NAME"])-1)] + str(row[115:140].rtrip())) 
                else:
            ...
            if wip_status==True and not row=="\n" and not row.startswith(tuple(skip_line_startwith_list)) and not (list(filter(row.__contains__, skip_line_contains_list)) != []):
                ...
                #print(row)
                if len(row[0:8].strip())==0 :
                    if len(row[16:41].strip())>0:
                        dict_status["PRINT-TIME"][(len(dict_status["PRINT-TIME"])-1)] = (dict_status["PRINT-TIME"][(len(dict_status["PRINT-TIME"])-1)] + str(row[16:41].strip()))
                    if len(row[50:74].strip())>0:
                        dict_status["COMMENT"][(len(dict_status["COMMENT"])-1)] = (dict_status["COMMENT"][(len(dict_status["COMMENT"])-1)]+' ' + str(row[50:74])) 
                    if len(row[9:15].strip())>0:
                        ...
                        if len(sts.strip())>0:
                          ...
                          dict_status["STATUS"][(len(dict_status["STATUS"])-1)] = (dict_status["STATUS"][(len(dict_status["STATUS"])-1)] + str(row[9:15].strip())) 
                    if len(row[86:].strip())>0:
                        dict_status["EPDE.SPC.INS"][(len(dict_status["EPDE.SPC.INS"])-1)] = (dict_status["EPDE.SPC.INS"][(len(dict_status["EPDE.SPC.INS"])-1)] +' '+ str(row[86:]))
                  
                
                else:
                    dict_status["REFER"].append(str(row[0:8].strip()))
                    dict_status["STATUS"].append(str(row[9:15].strip()))
                    dict_status["PRINT-TIME"].append(str(row[16:41].strip()))
                    dict_status["CLOSED"].append(str(row[42:49].strip()))
                    dict_status["COMMENT"].append(str(row[50:74]))
                    dict_status["HOME PHONE"].append(str(row[75:85].strip()))
                    dict_status["EPDE.SPC.INS"].append(str(row[86:]))
            ...
            if wip_opcode==True and not row=="\n" and not row.startswith(tuple(skip_line_startwith_list)) and not (list(filter(row.__contains__, skip_line_contains_list)) != []):
                if len(row[0:8].strip())==0 :
                    if len(row[9:].strip())>0:
                        dict_opcodes["OPCODE"][(len(dict_opcodes["OPCODE"])-1)] = (dict_opcodes["OPCODE"][(len(dict_opcodes["OPCODE"])-1)] + str(row[9:].strip()))
            
                else:
                    dict_opcodes["REFER"].append(str(row[0:8].strip()))
                    dict_opcodes["OPCODE"].append(str(row[9:].strip()))
        ...
        if loglevel==logging.DEBUG:
            ...
            logger.debug("Parse Completed...dict_opcodes:"+str(dict_opcodes))
        ...
        df_opcodes = pad.DataFrame(dict_opcodes)
        
        df_opcodes = df_opcodes.replace(np.nan,'', regex=True)      
        ...
        return {'df_final':df_final,'df_opcodes':df_opcodes,'account_line':account_line}
    @classmethod
