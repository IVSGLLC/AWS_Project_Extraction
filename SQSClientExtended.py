...
class SQSClientExtended(object):
  ...
  def __delete_message_payload_from_s3(self, receipt_handle):
    try:
      ...
      s3_object.delete()
      self.logger.debug("Deleted s3 object https://s3.amazonaws.com/"+str(s3_msg_bucket_name)+"/"+str(s3_msg_key))
    except Exception as e:
      self.logger.error("Failed to delete the message content in S3 object "+ str(e))
      raise e
  ...
  def delete_message(self, queue_url, receipt_handle):
    ...
    self.logger.debug("receipt_handle="+str(receipt_handle))
  ...
  def send_message(self, queue_url, message, message_group_id=None, message_deduplication_id=None, message_attributes={}):
    ...
    if not all([message_group_id, message_deduplication_id]):
      if any([message_group_id, message_deduplication_id]):
        ...
        fifo_queue = False
    self.logger.debug("message_attributes="+str(message_attributes))
    msg_attributes_size = self.__get_msg_attributes_size(message_attributes)
    ...
    self.logger.debug("large_payload_attribute_value="+str(large_payload_attribute_value))
    ...
    if self.always_through_s3 or self.__is_large(str(message), message_attributes):
      ...
      message_attributes[SQSExtendedClientConstants.RESERVED_ATTRIBUTE_NAME.value] = {'StringValue': str(self.__get_string_size_in_bytes(str(message))), 'DataType': 'Number'}
      ...
      s3_key_message=json.dumps(message,sort_keys=True)   
      return self._send_queue_message(queue_url, s3_key_message, message_group_id, message_deduplication_id, message_attributes, fifo_queue)  
  ...
  def __store_message_in_s3(self, message_body):
    ...
    try:
      ...
      self.logger.error("Failed to store the message content in an S3 object. SQS message was not sent.")
      raise e
